...
static void read_from_mpl(D_GyroDataTypeDef *gyro_data)
{
    long msg,data_temp[4];
    int8_t accuracy;
    unsigned long timestamp;
    float float_data[3] = {0};

    if (inv_get_sensor_type_quat(data_temp, &accuracy, (inv_time_t*)&timestamp))
    {

        for(int i = 0; i < 4; i++)
        {
            gyro_data->quat_data[i] = data_temp[i]/65536.f;

        }
        memset(data_temp, 0, sizeof(data_temp));
    }

    if (inv_get_sensor_type_accel(data_temp, &accuracy,
        (inv_time_t*)&timestamp))
    {

        for(int i = 0; i < 3; i++)
        {
            gyro_data->acceleration_speed[i] = data_temp[i]/65536.f;
        }
        memset(data_temp, 0, sizeof(data_temp));
    }

    if (inv_get_sensor_type_gyro(data_temp, &accuracy,
    (inv_time_t*)&timestamp))
    {

        for(int i = 0; i < 3; i++)
        {
            gyro_data->gyro_speed[i] = data_temp[i]/65536.f;

        }
        memset(data_temp, 0, sizeof(data_temp));
    }

    if (inv_get_sensor_type_euler(data_temp, &accuracy,
       (inv_time_t*)&timestamp))
     {
        for(int i = 0; i < 3; i++)
         {
            gyro_data->euler_angle[i] = data_temp[i]/65536.f;
         }
        memset(data_temp, 0, sizeof(data_temp));
     }
#ifdef COMPASS_ENABLED
    if (inv_get_sensor_type_compass(data_temp, &accuracy,
    (inv_time_t*)&timestamp))
    {

       for(int i = 0; i < 3; i++)
       {
            gyro_data->compass[i] = data_temp[i]/65536.f;
       }
       memset(data_temp, 0, sizeof(data_temp));
    }
    ...
    msg = inv_get_message_level_0(INV_MSG_MOTION_EVENT |
       ...
       if (msg)
       {
           ...
           {
               gyro_data->nomotion = true;
           }
       }

       gyro_data->lock_state = d_is_lock;
}
...
static void tap_cb(unsigned char direction, unsigned char count)
{
   ...
   MPL_LOGI("x%d\n", count);
   static unsigned long timer_temp = 0;
   if((timestamp - timer_temp) > 1500)
     {
       timer_temp = timestamp;
       d_is_lock = d_is_lock?false:true;
     }
...
//       if(INV_SUCCESS != inv_set_no_motion_time(240000))
//         {
//
//           trace_printf("err\n");
//         }
      ...
      /* Allows use of the MPL APIs in nread_from_mpl. */
...
/**
* @brief   d_gyro_data_get  将陀螺仪的所有原始数据和欧拉角数据一同存入传入的结构体中
*           欧拉角是需要在函数中进行运算的，所以函数会有一定的延时。
* @param  D_GyroDataTypeDef *data
* @retval   D_OK
* @retval   D_ERR
*/
bool d_gyro_data_get(D_GyroDataTypeDef *gyro_data)
{
...
{
   mpu_lp_motion_interrupt(600, 1,20);
